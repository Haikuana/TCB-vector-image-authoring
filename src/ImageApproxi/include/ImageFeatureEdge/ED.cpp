...
{	
	...
	//bilateralFilter(srcImage, smoothImage, 7, 10, 20);
	//medianBlur(srcImage, smoothImage, 11);
	...
	for (int i = 1; i<height - 1; i++) {
		for (int j = 1; j<width - 1; j++) {
			...
			case SOBEL_OPERATOR:
				...
				/*gx_up = abs(2 * (smoothImg[i*width + j + 1] - smoothImg[i*width + j - 1]) +
					1.5 * (smoothImg[(i+1)*width + j + 1] - smoothImg[(i + 1)*width + j - 1]));
				gx_down = abs(2 * (smoothImg[i*width + j + 1] - smoothImg[i*width + j - 1]) +
					1.5 * (smoothImg[(i - 1)*width + j + 1] - smoothImg[(i - 1)*width + j - 1]));
				gx = std::max(gx,gx_up); gx = std::max(gx, gx_down);

				gy_up = abs(2 * (smoothImg[(i + 1)*width + j] - smoothImg[(i - 1)*width + j]) +
					1.5 * (smoothImg[(i + 1)*width + (j+1)] - smoothImg[(i - 1)*width + (j+1)]));
				gy_down = abs(2 * (smoothImg[(i + 1)*width + j] - smoothImg[(i - 1)*width + j]) +
					1.5 * (smoothImg[(i + 1)*width + (j - 1)] - smoothImg[(i - 1)*width + (j - 1)]));
				gy = std::max(gy, gy_up); gy = std::max(gy, gy_down);*/
...
void ED::ComputeAnchorPoints()
{
	...
	for (int i = 2; i<height - 2; i++) {
		...
		for (int j = start; j<width - 2; j += inc) {
			...
			//test
			if (j>=142 && j <=143 &&i==369)
			{
				int t = 1;
			}

			int gradnow = gradImg[i*width + j];
